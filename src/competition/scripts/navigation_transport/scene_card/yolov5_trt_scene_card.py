"""
rosservice call /yolo_scene_card_service/recognize_card_1
rosservice call /yolo_scene_card_service/recognize_card_2
rosservice call /yolo_scene_card_service/recognize_card_3
...
class YoLov5TRT(object):
    def __init__(self, engine_file_path, plugin, classes, conf_thresh=0.8, iou_threshold=0.4):
        ...
        for binding in engine:
            ...
            if engine.binding_is_input(binding):
                ...
                self.output_len = trt.volume(engine.get_binding_shape(binding))
                self.max_detections = 1000
                self.result_elem_len = (self.output_len - 1) // self.max_detections
    ...
    def infer(self, raw_image_generator):
        ...
        for i in range(self.batch_size):
            result_boxes, result_scores, result_classid = self.post_process(
                output[i * self.output_len: (i + 1) * self.output_len],
                batch_origin_h[i],
                batch_origin_w[i],
            )
    ...
    def post_process(self, output, origin_h, origin_w):
        num = min(int(output[0]), self.max_detections)
        pred = np.reshape(output[1:], (-1, self.result_elem_len))[:num, :]
        if self.result_elem_len > 6:
            pred = pred[:, :6]
        # Do nms
...
if __name__ == "__main__":
    ...
    classes_file = os.path.join(model_dir, 'classes.names')
    with open(classes_file, 'r', encoding='utf-8') as fp:
        classes = [line.strip() for line in fp if line.strip()]
    yolov5_wrapper = YoLov5TRT(
        os.path.join(model_dir, 'scene_card.engine'),
        os.path.join(model_dir, 'scene_card_libmyplugins.so'),
        classes,
    )
    try:
        ...
        while True:
            ...
            if ret:
                ...
                print('detections:', len(boxes), list(zip(classid, scores)))
                break
    finally:
